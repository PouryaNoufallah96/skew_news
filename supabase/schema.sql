-- SKEW database schema (AGENTS.md §7).
-- Apply in Supabase Dashboard → SQL Editor. The `embedding` column on
-- article_analyses is added later in §20 after pgvector is enabled — not here.
...
create table if not exists public.sources (
  ...
  name text not null,
  listing_url text not null unique, -- homepage entry page (§9/§11)
...
-- all access is via the server service-role client (bypasses RLS). See AGENTS.md §21.
